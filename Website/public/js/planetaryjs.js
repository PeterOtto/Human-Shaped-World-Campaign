...
(function (root, factory) {
      ...
      planet.onDraw(function() {
        planet.withSavedContext(function(context) {
          ...
          if (config.stroke) {
            ...
            if (config.dash){
              var dash = parseInt(config.dash) || 0;
              var space = parseInt(config.space) || 0;
              context.setLineDash([dash,space]);
            }
            context.stroke();
          }
        });
  ...
  planetaryjs.plugins.pings = function(config) {
    ...
    var addPing = function(lng, lat, options) {
      options = options || {};
      options.strokeColor = options.strokeColor || config.strokeColor || 'white';
    ...
    var drawPing = function(planet, context, now, alive, ping) {
      ...
      var strokeColor = d3.rgb(ping.options.strokeColor);
      strokeColor = "rgba(" + strokeColor.r + "," + strokeColor.g + "," + strokeColor.b + "," + alpha + ")";
      context.strokeStyle = strokeColor;
      var outline = d3.geo.circle().origin([ping.lng, ping.lat])
  ...
  planetaryjs.plugins.dots = function(config) {
    ...
    var addDot = function(lng, lat, dotId, options) {
      options = options || {};
      options.fillColor = options.fillColor || config.fillColor || 'white';
      options.size = options.size || config.size || 0.2;
      var dot = { options: options };
      dot.lng = lng;
      dot.lat = lat;
      dot.id = dotId;
      dots.push(dot);
    ...
    var drawDot = function(planet, context, dot) {
      var alpha = 1;
      var fillColor = d3.rgb(dot.options.fillColor);
      fillColor = "rgba(" + fillColor.r + "," + fillColor.g + "," + fillColor.b + "," + alpha + ")";
      context.fillStyle = fillColor;
      var dotFill = d3.geo.circle().origin([dot.lng, dot.lat])
        .angle(dot.options.size)();
      context.beginPath();
      planet.path.context(context)(dotFill);
      context.fill();
    };
